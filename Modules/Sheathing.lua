...
-- Own event frame: registering these on the shared addon object used to clobber the
-- core PLAYER_ENTERING_WORLD handler (and Unload() removed it entirely).
LingkanUI:EmbedEvents(LingkanUI.Sheathing)

function LingkanUI.Sheathing:Load()
    local function Handle() LingkanUI:SheatHandler() end

    self:RegisterEvent("PLAYER_ENTERING_WORLD", Handle)
    self:RegisterEvent("UNIT_TARGET", Handle)
    self:RegisterEvent("UNIT_MODEL_CHANGED", Handle)
