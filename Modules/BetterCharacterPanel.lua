...
--------------------------------------- Character Panel Scale ---------------------------------------

-- Bounds are exposed on the module so the options slider stays in sync with the clamp
local MIN_PANEL_SCALE, MAX_PANEL_SCALE, DEFAULT_PANEL_SCALE = 0.5, 2, 1
LingkanUI.BetterCharacterPanel.MIN_SCALE = MIN_PANEL_SCALE
LingkanUI.BetterCharacterPanel.MAX_SCALE = MAX_PANEL_SCALE
LingkanUI.BetterCharacterPanel.DEFAULT_SCALE = DEFAULT_PANEL_SCALE

local scaleHooked = false

local function GetConfiguredScale()
    local profile = LingkanUI.db and LingkanUI.db.profile and LingkanUI.db.profile.betterCharacterPanel
    local scale = (profile and profile.scale) or DEFAULT_PANEL_SCALE
    -- Guard against a hand-edited or corrupted saved variable making the panel unusable
    return math.min(MAX_PANEL_SCALE, math.max(MIN_PANEL_SCALE, scale))
end

-- Applies the configured scale to the Blizzard character panel. Deliberately independent
-- of the module's enabled state: it only resizes the panel and does not rely on the
-- item level / enchant / socket display.
function LingkanUI.BetterCharacterPanel:ApplyScale()
    local frame = _G.CharacterFrame
    if not frame then return end

    local scale = GetConfiguredScale()
    DebugPrint("Applying character panel scale: " .. tostring(scale))
    frame:SetScale(scale)

    -- Addons skinning the panel can reset its scale, so re-assert it whenever it opens
    if not scaleHooked then
        scaleHooked = true
        frame:HookScript("OnShow", function(self)
            self:SetScale(GetConfiguredScale())
        end)
    end
end
