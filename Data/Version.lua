...
-- Resolved immediately: these only depend on the interface number, and modules read
-- them at file-load time (BetterCharacterPanel branches on isMop while loading), which
-- is long before :Init() runs on FIRST_FRAME_RENDERED.
versionUtil.isRetail = interface >= 110000
versionUtil.isClassicEra = interface < 40000
versionUtil.isMop = interface >= 50000 and interface < 60000

-- Needs a player aura, so it stays false until :Init()
...
function versionUtil:Init()
    -- Static flags are already resolved at load time; only Remix needs the player aura
