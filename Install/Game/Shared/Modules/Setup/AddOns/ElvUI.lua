...
do
    if LUI:IsAddOnEnabled("ElvUI") then
        E = unpack(ElvUI)
    end
end
...
local function ImportElvUI(addon, resolution)
    ...
    if not data or type(data) ~= "table" then
        LUI:Print("An error occured while decompressing the profile.")

        return
    end

    SE.CompleteSetup(addon)
    DI:SetImportedProfile(profileType, "LingkanUI", data, true)
    E:SetupCVars(true)
...
function SE.ElvUI(addon, import, resolution)
    ...
    if LUI.Retail then
        ...
        E.private.WT.skins.bigWigsSkin.queueTimer.countDown.name = "LingkanUI"
        ...
        E.private.WT.skins.ime.label.name = "LingkanUI"
    ...
    E.private.general.chatBubbleFont = "LingkanUI"
    ...
    E.private.general.namefont = "LingkanUI"
