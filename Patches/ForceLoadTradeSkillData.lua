...
hack:SetScript('OnEvent', function(self, event)
    if event == 'PLAYER_LOGIN' or event == 'SKILL_LINES_CHANGED' then
        ...
        print("ForceLoadTradeSkillData: Attempting to force load tradeskill data.")

        local professionIDs = self:GetAvailableProfessionIDs()
...
function hack:OnKeyDown()
    ...
    -- check if 202 is in the list of professions
    local professionID = self.professionIDs[1]
    for _, id in next, self.professionIDs do
        if id == 202 then
            professionID = 202
            break
        end
...
function hack:GetAvailableProfessionIDs()
    -- any profession except archaeology is valid for requesting data
    local professionIDs = {}
    for index, professionIndex in next, { GetProfessions() } do
        if index ~= 3 and professionIndex then
            local _, _, _, _, _, _, professionID = GetProfessionInfo(professionIndex)
            if professionID then
                table.insert(professionIDs, professionID)
            end
        end
    end
